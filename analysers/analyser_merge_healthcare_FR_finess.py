...
class SubAnalyser_Merge_Healthcare_FR_Finess(SubAnalyser_Merge_Dynamic):
    def __init__(self, config, error_file, logger, srid, is_in, categories, items, missing_osm, classs, level, title, tags_select, tags_generate1, tags_generate2):
        self.missing_official = {"item":str(items+2), "class": classs+1, "level": level, "tag": ["merge"], "desc": T_f(u"{0} not integrated", title) }
        if missing_osm != False:
            self.missing_osm = {"item":str(items+3), "class": classs+2, "level": level, "tag": ["merge"], "desc": T_f(u"{0} without tag \"{1}\" or invalid", title, 'ref:FR:FINESS') }
        self.possible_merge = {"item":str(items+4), "class": classs+3, "level": level, "tag": ["merge"], "desc": T_f(u"{0}, integration suggestion", title) }
