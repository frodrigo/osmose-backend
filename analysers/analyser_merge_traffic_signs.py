...
class SubAnalyser_Merge_Traffic_Signs(SubAnalyser_Merge_Dynamic):
    def __init__(self, config, error_file, logger, item, classs, level, tags, otype, conflation, title, object, selectTags, generateTags, mapping, source, layer):
        ...
        self.init(
            ...
            # config.country
            CSV(Source(attribution = "Mapillary Traffic Signs", fileUrl = f"http://proxy.osmose.openstreetmap.fr/mapillary/mapillary_signs_2023_03_06-{config.osmosis_manager.db_schema}.csv.bz2", bz2 = True)),
            Load_XY("lon", "lat",
                select = {"value": object}),
            Conflate(
                select = Select(
