...
class SubAnalyser_Merge_Street_Objects(SubAnalyser_Merge_Dynamic):
    def __init__(self, config, error_file, logger, classs, level, otype, conflation, title, object, selectTags, generateTags, mapping, source, layer):
        ...
        self.init(
            ...
            CSV(Source(attribution = "Mapillary Street Objects", fileUrl = f"http://proxy.osmose.openstreetmap.fr/mapillary/mapillary_objects_2023_03_06-{config.osmosis_manager.db_schema}.csv.bz2", bz2 = True)),
            Load_XY("lon", "lat",
                select = {"value": object}),
            Conflate(
                select = Select(
